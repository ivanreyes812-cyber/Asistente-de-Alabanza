...
# We will create manantialSongs.ts directly
# Let's define the comprehensive dataset for all 272 songs in Manantial de Inspiración.

def main():
    songs = []
    
    # 1 to 272
    # We populate each hymn with accurate details from the PDF text.

    # Helper function to add song
    def add(num, title, category, key, bpm, author, lyrics):
        songs.append({
            "id": f"manantial-{num}",
            "number": num,
            "title": title,
    ...
    # Songs 1 to 20
    add(1, "Por Fe", "Doctrina", "C", 90, "Juan Carlos Pérez", """I
...
Y los [G]hombres desobedientes al fuego se i[C]rán//""")
    ...
    add(2, "Quiero Adorarte", "Adoración", "D", 72, "Juan Carlos Pérez", """[D]Digno de Honra, [A]digno de Gloria [Bm]y de adoración
...
[G]Toma mi [A7]cora[D]zón.""")
    ...
    add(3, "Sin Tu Amor", "Adoración", "Em", 68, "Guillermo Cabezas / Orlando Roa", """I
...
Y tu Pa[B7]labra me guía y me da feli[Em]cidad.""")
    ...
    add(4, "Hay Poder en el Nombre", "Alabanza", "G", 110, "Juan Carlos Pérez", """I
...
En el [D7]nombre de Je[G]sús.""")
    ...
    add(5, "Quiero Llegar", "Adoración", "C", 75, "Edgar Devia", """I
...
De [G7]gracia y de eterno so[C]laz.""")
    ...
    add(6, "Nuevo Amanecer", "Alabanza", "D", 100, "Guillermo Cabezas / Orlando Roa", """I
...
En la [A7]paz que hay en el rostro de aquel que vive fe[D]liz.""")
    ...
    add(7, "Tu Nombre", "Adoración", "G", 72, "Jhon Carlos Mena / Orlando Roa", """I
...
[C]Nombre sobre [D7]todo nombre, es [C]Tu Nom[D7]bre Je[G]sús.""")
    ...
    add(8, "Mensaje de Amor", "Evangelismo", "G", 88, "Edgar Devia", """I
...
De ofre[D7]cernos amor y favores.""")
    ...
    add(9, "Gracias por Todo Jesús", "Agradecimiento", "D", 80, "Luz E. Cifuentes", """I
...
Que [G]debes po[A7]ner tu con[D]fianza.""")
    ...
    add(10, "Caña Cascada", "Consolación", "Em", 72, "Luz E. Cifuentes", """I
...
Sólo en Ti, éste, mi o[B7]caso, amanece[Em]rá.""")
    ...
    print(f"Generated first batch. Total songs: {len(songs)}")
